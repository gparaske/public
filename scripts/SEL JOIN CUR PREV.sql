...
SET @ReportingDate = '2024-03-31'--'"+ @[User::varReportingDate] +"';
...
DELETE FROM dbo.IFD_SUB WHERE OBS_A_ID = 'GR011' AND REF_DT='2024-03-31' AND INSTST <> 0 AND SOURCE_SYSTEM_ID IN ('INSTI_1','INSTI_2','INSTI_3')
UPDATE B
SET B.QUALIFICATION_FLAG = 0
  , B.INSTST = 0
  , B.SOURCE_SYSTEM_ID = 'DELETED'--δεν κρατάμε κάπου την τελευταία τιμή
FROM dbo.IFD_SUB B
WHERE B.OBS_A_ID = 'GR011' AND B.REF_DT='2024-03-31' AND B.SOURCE_SYSTEM_ID IN ('INSTU_1','INSTU_2','INSTU_3')

DECLARE @MAX_RECID BIGINT = (SELECT MAX(TRY_CONVERT(BIGINT, RECID)) FROM dbo.IFD_SUB WHERE OBS_A_ID = 'GR011' AND REF_DT='2024-03-31')
...
SELECT '2024-03-31' AS [REF_DT]
      ...
      ,CONCAT(REPLACE(CAST(CAST('2024-03-31' AS DATE) AS NVARCHAR),'-',''), '01') AS [DATA_VERSION]
      ,A.[OPB]
      ,A.[OOB]
      ,A.[AMP]
      ,4 AS [INSTST]
...
FROM dbo.IFD_SUB A
LEFT JOIN dbo.IFD_SUB B ON B.OBS_A_ID=A.OBS_A_ID AND B.REF_DT='2024-03-31' AND B.CNID=A.CNID AND B.INSID=A.INSID
WHERE A.OBS_A_ID = 'GR011' AND A.REF_DT=EOMONTH('2024-03-31',-1) 
AND (
...
--Άρα εφόσον θα χάνεται η μια αποστολή, τους επόμενους μήνες θα βρίσκει μόνο τις περιπτώσεις που γύρισαν σε INSTST=3 μέσα στο τρίμηνο.
	 (MONTH('2024-03-31') NOT IN (1,4,7,10) AND A.QUALIFICATION_FLAG=1 AND A.INSTST=3 AND A.SOURCE_SYSTEM_ID IN ('INSTI_1','INSTI_2','INSTU_1','INSTU_2'))
    )
...
WHERE A.OBS_A_ID = 'GR011' AND A.REF_DT='2024-03-31' AND A.INSTST <> 0 AND A.SOURCE_SYSTEM_ID IN ('INSTI_1','INSTI_2','INSTI_3','INSTU_1','INSTU_2','INSTU_3')
...
LEFT JOIN src.EDW_com_FX_RATES fx WITH(NOLOCK) ON fx.FX_CCY_DESCR=A.CUR AND fx.DT_REC<='2024-03-31' AND fx.FX_DATE>'2024-03-31'
WHERE A.OBS_A_ID = 'GR011' AND A.REF_DT='2024-03-31' AND A.INSTST <> 0 AND A.SOURCE_SYSTEM_ID IN ('INSTI_1','INSTI_2','INSTI_3')
...
, A.TRAM=(
	...
	ELSE (
		CASE 
		WHEN EDW_LEND_ACC_LOAN_SCRZ.ACC_ID IS NOT NULL AND ISNULL(EDW_LEND_ACC_LOAN_SCRZ.DT_EXIT,'9999-12-31')>'2024-03-31'
...
, A.PDD=(
	...
	ELSE ISNULL(DATEADD(day, EDW_LEND_ACC_DPD_DLQ_SMY_M.DLY_DAYS*(-1)+1, '2024-03-31'),'1900-01-01')
	END
)
--	AS PDD
, A.INTRRD=(
	...
	WHEN cofnc.ACC_ID IS NOT NULL AND COF.OBS_A_ID IS NOT NULL--Co-Financed (2nd Row)
	THEN NULL--Co-Financed
    WHEN ISNULL(EDW_LEND_ACC_LOAN_RATES.DT_BASE_RATE_NXT_RVSN,'1111-11-11') >= '2024-03-31'
...
LEFT JOIN src.EDW_com_FX_RATES fx WITH(NOLOCK) ON fx.FX_CCY_DESCR=A.CUR AND fx.DT_REC<='2024-03-31' AND fx.FX_DATE>'2024-03-31'
LEFT JOIN src.EDW_LEND_ACC_LOAN_BAL_EOD bal WITH(NOLOCK) ON bal.ACC_ID = A.ACC_ID AND bal.DT_EFF<='2024-03-31' AND bal.DT_END>'2024-03-31'
LEFT JOIN src.EDW_lend_ACC_COFNC cofnc ON cofnc.ACC_ID = A.ACC_ID AND cofnc.DT_END>EOMONTH('2024-03-31',-1) and cofnc.DT_EFF<=EOMONTH('2024-03-31',-1) AND cofnc.RCD_STS=1 AND cofnc.SENIORITY_PCT<>100
LEFT JOIN src.EDW_acc_ACC ACC ON ACC.ACC_ID = A.ACC_ID AND ACC.DT_END>'2024-03-31' and ACC.DT_EFF<='2024-03-31'
--LEFT JOIN EDW_UAT.lend.ACC_LOAN_FIN_DTL DTL WITH(NOLOCK) ON DTL.ACC_ID = A.ACC_ID AND DTL.DT_EFF<='2024-03-31' AND DTL.DT_END>'2024-03-31'
LEFT JOIN #ACC_LOAN_FIN_DTL DTL ON DTL.ACC_ID = A.ACC_ID
--LEFT JOIN EDW_UAT.lend.ACC_LOAN_TRN_DTL ACC_LOAN_TRN_DTL WITH(NOLOCK) ON ACC_LOAN_TRN_DTL.ACC_ID = A.ACC_ID AND ACC_LOAN_TRN_DTL.DT_ACCOUNTING='2024-03-31'--ΘΕΛΕΙ SUM ΥΠΑΡΧΕΙ Ο ΠΙΝΑΚΑΣ ##ACC_LOAN_TRN_DTL_P8
...
LEFT JOIN (
	...
	WHERE EDW_LEND_ACCRUALS.DT_REC<='2024-03-31'
...
LEFT JOIN src.EDW_lend_ACC_LOAN_SCRZ EDW_lend_ACC_LOAN_SCRZ ON EDW_LEND_ACC_LOAN_SCRZ.ACC_ID = A.ACC_ID
	AND EDW_LEND_ACC_LOAN_SCRZ.DT_EFF <= '2024-03-31'
	AND EDW_LEND_ACC_LOAN_SCRZ.DT_END > '2024-03-31'
...
LEFT JOIN src.EDW_LEND_ACC_DPD_DLQ_SMY_M  EDW_LEND_ACC_DPD_DLQ_SMY_M ON EDW_LEND_ACC_DPD_DLQ_SMY_M.ACC_ID = A.ACC_ID
	AND EDW_LEND_ACC_DPD_DLQ_SMY_M.DT_REC= '2024-03-31' 
...
LEFT JOIN src.EDW_lend_ACC_LOAN_RATES EDW_LEND_ACC_LOAN_RATES ON EDW_LEND_ACC_LOAN_RATES.ACC_ID = A.ACC_ID
 ...
 AND EDW_LEND_ACC_LOAN_RATES.DT_EFF <= '2024-03-31'
 AND EDW_LEND_ACC_LOAN_RATES.DT_END > '2024-03-31'
...
WHERE OBS_A_ID='GR011' AND REF_DT='2024-03-31' AND PDD IS NOT NULL AND PDD<>'1900-01-01' AND PDD<STLD AND SOURCE_SYSTEM_ID IN ('INSTI_1','INSTI_2','INSTI_3')
...
-- SELECT *
FROM dbo.IFD_SUB B
LEFT JOIN dbo.IFD_SUB A ON A.OBS_A_ID=B.OBS_A_ID AND A.REF_DT=EOMONTH('2024-03-31',-1) AND B.CNID=A.CNID AND B.INSID=A.INSID 
WHERE B.OBS_A_ID = 'GR011' AND B.REF_DT='2024-03-31' 
AND (
     (A.QUALIFICATION_FLAG=1 AND A.INSTST=0)--τον προηγούμενο μήνα το είχαμε στείλει ως ενεργή
	 OR
...
SELECT ACC_ID 
FROM EDW.lend.ACC_LOAN_EARTH_SUPPL_FIN WITH(NOLOCK) WHERE DT_REF BETWEEN DATEADD(DAY,1,EOMONTH('2024-03-31',-1)) AND '2024-03-31'
...
LEFT JOIN dbo.IFD_SUB D ON D.OBS_A_ID=A.OBS_A_ID AND D.REF_DT=EOMONTH('2024-03-31',-1) AND D.CNID=A.CNID AND D.INSID=A.INSID 
WHERE A.OBS_A_ID = 'GR011' AND A.REF_DT='2024-03-31' AND A.SOURCE_SYSTEM_ID IN ('INSTI_1','INSTI_2','INSTI_3','INSTU_1','INSTU_2','INSTU_3')
...
AND PARALIST_ID=4
AND (
     (DT_EFF BETWEEN DATEADD(DAY,1,EOMONTH('2024-03-31',-3)) AND '2024-03-31')
	 OR
     (DT_END BETWEEN DATEADD(DAY,1,EOMONTH('2024-03-31',-3)) AND '2024-03-31')
    )
-- EDW -> Anacredit (TEST-END)
...
FROM dbo.IFD_SUB A
LEFT JOIN src.EDW_acc_ACC B ON B.ACC_ID = TRY_CONVERT(BIGINT, A.ACC_ID) AND B.ACC_LIFE_STATUS_ID = 5
WHERE A.OBS_A_ID = 'GR011' AND A.REF_DT='2024-03-31' AND ISNUMERIC(A.ACC_ID)=1
